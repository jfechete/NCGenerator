...
class Generator:
    ...
    def add_multipass(self, paths, depth, depth_step = DEPTH_STEP):
        ...
        Each path increases in depth by the depth_step parameter until it reaches the desired depth.
        """
        current_depth = 0
    ...
    def add_pass(self, paths, depth):
        """
        Creates a single pass that follows the given Paths object at the supplied depth.
        """
        for path in paths:
    ...
    def _is_move_far(self, pos_a, pos_b):
        return (pos_a[0]-pos_b[0])**2 + (pos_a[1]-pos_b[1])**2 >= self._min_move_dist**2
